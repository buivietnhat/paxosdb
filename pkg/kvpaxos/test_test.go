...
func check(t *testing.T, ck *Clerk, key string, value string) {
  v := ck.Get(key)
  if v != value {
    logger.Debug(DTest, "Get(%v) -> %v, expected %v", key, v, value)
...
func TestBasic(t *testing.T) {
  ...
  for i := 0; i < nservers; i++ {
    ...
    kva[i] = StartServer(kvh, i, sLogger)
  }
  ...
  logger.Debug(DTest, "Test: Basic put/append/get ...\n")
  ...
  logger.Debug(DTest, "Test: Concurrent clients ...\n")
...
func part(t *testing.T, tag string, npaxos int, p1 []int, p2 []int, p3 []int) {
  ...
  logger.Debug(DTest, "Parition into p1: %v, p2: %v, p3: %v", p1, p2, p3)
  ...
  for pi := 0; pi < len(pa); pi++ {
    ...
    for i := 0; i < len(p); i++ {
      for j := 0; j < len(p); j++ {
        ...
        err := os.Link(pj, ij)
        if err != nil {
          logger.Debug(DTest, "os.Link(%v, %v): %v", pj, ij, err)
...
func TestPartition(t *testing.T) {
  ...
  logger.Debug(DTest, "Test: No partition ...\n")
  ...
  logger.Debug(DTest, "Test: Progress in majority ...\n")
  ...
  logger.Debug(DTest, "Test: No progress in minority ...\n")
  ...
  select {
  case <-done0:
    logger.Debug(DTest, "Put in minority completed")
    t.Fatalf("Put in minority completed")
  case <-done1:
    logger.Debug(DTest, "Get in minority completed")
  ...
  logger.Debug(DTest, "Test: Completion after heal ...\n")
  ...
  case <-time.After(30 * 100 * time.Millisecond):
    logger.Debug(DTest, "Put did not complete")
  ...
  case <-time.After(100 * 100 * time.Millisecond):
    logger.Debug(DTest, "Get did not complete")
...
func checkAppends(t *testing.T, v string, counts []int) {
  ...
  for i := 0; i < nclients; i++ {
    ...
    for j := 0; j < counts[i]; j++ {
      ...
      if off < 0 {
        logger.Debug(DTest, "missing element in Append result")
        t.Fatalf("missing element in Append result")
      }
      off1 := strings.LastIndex(v, wanted)
      if off1 != off {
        logger.Debug(DTest, "duplicate element in Append result")
        t.Fatalf("duplicate element in Append result")
      }
      if off <= lastoff {
        logger.Debug(DTest, "wrong order for element in Append result")
...
func TestUnreliable(t *testing.T) {
  ...
  logger.Debug(DTest, "Test: Basic put/get, unreliable ...\n")
  ...
  logger.Debug(DTest, "Test: Sequence of puts, unreliable ...\n")
  fmt.Printf("Test: Sequence of puts, unreliable ...\n")

  for iters := 0; iters < 6; iters++ {
    logger.Debug(DTest, "========================================= iteration %d =========================================", iters)
    const ncli = 5
    ...
    for cli := 0; cli < ncli; cli++ {
      ...
      go func(me int) {
        ...
        getv := myck.Get(key)
        if getv != vv {
          logger.Debug(DTest, "wrong value1 for key %v, expect %v got %v", key, vv, getv)
          t.Fatalf("wrong value")
        }
        getv1 := myck.Get(key)
        if getv1 != vv {
          logger.Debug(DTest, "wrong value2 for key %v, expect %v got %v, old one %v", key, vv, getv1, getv)
          t.Fatalf("wrong value")
        }
        ok = true
      }(cli)
  ...
  logger.Debug(DTest, "Test: Concurrent clients, unreliable ...\n")
  ...
  for iters := 0; iters < 20; iters++ {
    ...
    for i := 0; i < nservers; i++ {
      va[i] = cka[i].Get("b")
      if va[i] != va[0] {
        logger.Debug(DTest, "mismatch; 0 got %v, %v got %v", va[0], i, va[i])
  ...
  logger.Debug(DTest, "Test: Concurrent Append to same key, unreliable ...\n")
  ...
  for i := 0; i < ncli; i++ {
    n := <-cha[i]
    if n < 0 {
      logger.Debug(DTest, "client failed")
...

  {
    for i := 0; i < nservers; i++ {
      vi := cka[i].Get("k")
      if vi != vx {
        logger.Debug(DTest, "mismatch; 0 got %v, %v got %v", vx, i, vi)
...
func TestHole(t *testing.T) {
  ...
  logger.Debug(DTest, "Test: Tolerates holes in paxos sequence ...\n")
  ...
  for iters := 0; iters < 5; iters++ {
      ...
      go func(cli int) {
        ...
        for atomic.LoadInt32(&done) == 0 {
          ...
          if (rand.Int() % 1000) < 500 {
            ...
            v := cka[ci].Get(key)
            if v != last {
              logger.Debug(DTest, "%v: wrong value, key %v, wanted %v, got %v",
                cli, key, last, v)
    ...
    if ok == false {
      logger.Debug(DTest, "something is wrong")
...
func TestManyPartition(t *testing.T) {
  ...
  logger.Debug(DTest, "Test: Many clients, changing partitions ...\n")
    ...
    go func(cli int) {
      ...
      for atomic.LoadInt32(&done) == 0 {
        if (rand.Int() % 1000) < 500 {
          ...
          v := myck.Get(key)
          if v != last {
            logger.Debug(DTest, "%v: get wrong value, key %v, wanted %v, got %v",
              cli, key, last, v)
