...
func TestForgetMem(t *testing.T) {
  ...
  logger.Debug(DTest, "Test: Paxos frees forgotten instance memory ...\n")
  ...
  for i := 0; i < npaxos; i++ {
    if pxa[i].Min() != 11 {
      logger.Debug(DTest, "expected Min() %v, got %v\n", 11, pxa[i].Min())
  ...
  if m2.Alloc > (m1.Alloc / 2) {
    logger.Debug(DTest, "memory use did not shrink enough, expected %d, got %d", m1.Alloc/2, m2.Alloc)
  ...
  for seq := 0; seq < npaxos && seq < 10; seq++ {
    ...
    for i := 0; i < npaxos; i++ {
      ...
      if fate != Forgotten {
        logger.Debug(DTest, "seq %d < Min() %d but not Forgotten", seq, pxa[i].Min())
...
func TestRPCCount(t *testing.T) {
  ...
  logger.Debug(DTest, "Test: RPC counts aren't too high ...\n")
  ...
  if total1 > expected1 {
    logger.Debug(DTest, "too many RPCs for serial Start()s; %v instances, got %v, expected %v", ninst1, total1, expected1)
  ...
  if total2 > expected2 {
    logger.Debug(DTest, "too many RPCs for concurrent Start()s; %v instances, got %v, expected %v",
